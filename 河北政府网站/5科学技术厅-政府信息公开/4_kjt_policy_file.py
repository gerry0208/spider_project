...
def get_data(name, url, page_num):
    file = open(f'科技厅政策文件_{name}.csv', 'a', encoding='utf-8', newline='')
    writer = csv.writer(file)
    writer.writerow(['标题', '时间', '内容', '一级类别', '来源网站', '访问地址', '附件链接'])

    domain_url = url.replace('1.html', '')

    for page in range(1, page_num + 1):

        url = domain_url + f'{page}.html'
...
get_data('规范性文件汇总表', 'https://kjt.hebei.gov.cn/www/xxgk2020/228104/228108/228110/d2f83e65-1.html', 4)
